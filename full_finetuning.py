...
class Prompter(object):
    ...
    def generate_prompt(
        ...
        if input != "":
            ...
            res = self.no_input_template["prompt"].format(instruction=instruction)
        if output:
            res = f"{res}{output}"
...
def train(
    ...
    cutoff_len: int = 300, # 512 in original alpaca 
    train_on_inputs: bool = True,  # if False, masks out inputs in loss
    add_eos_token: bool = False,
    group_by_length: bool = False,  # faster, but produces an odd training loss curve
    # PEFT / quantization parameters
    train_using_lora: bool = False,
    lora_r: int = 4,
    lora_alpha: int = 16,
